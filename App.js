...
function App() {
  ...
  //Code added to calculate the details of last expense(using list of all expenses)
  ...
  const calculateTotals = (expense) => {
    ...
    expense.participants.forEach(p => {
      ...
      totalOwed += parseFloat(p.amountOwed);
    });
  
    return {
      totalPaid: totalPaid.toFixed(5),
      totalOwed: totalOwed.toFixed(5),
  ...
  const contractAddress = "0x1A6E6275a08c343E3587EF6520fa2AAb80c55594"; // Paste the address recieved from Remix IDE here
    ...
    <div className="App">
      <header className="App-header">
        ...
        : (
          <div>
            <h2>Welcome, {name}</h2>
            {/* Here is the new code added for showing the last expense after Welcome at Top of UI page */}
            {lastExpense && (
              <div style={{ border: '1px solid #aaa', borderRadius: '8px', padding: '10px', marginBottom: '20px', backgroundColor: 'transparent',color:'#000000 !important' }}>
                  <h3>🧾 Last Expense</h3>
                  <p><strong>Label:</strong> {lastExpense.label}</p>
                  <p><strong>Date:</strong> {lastExpense.timestamp}</p>
                  <p><strong>Participants:</strong> {calculateTotals(lastExpense).participantsCount}</p>
                  <p><strong>Total Paid:</strong> {calculateTotals(lastExpense).totalPaid} ETH</p>
                  <p><strong>Total Owed:</strong> {calculateTotals(lastExpense).totalOwed} ETH</p>
              </div>
            )}
